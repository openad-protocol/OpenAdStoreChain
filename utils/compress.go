...
import (
	adError "AdServerCollector/core/errors"
	"bytes"
	"compress/gzip"
	"compress/lzw"
	"fmt"
...
type CompressionConfig struct {
	// Kind of the compression algorithm to be used
	Type string
...
func Compress(data []byte, config *CompressionConfig) ([]byte, error) {
	...
	if err != nil {
		return nil, adError.Wrapf("failed to create a compression writer: {{err}}", err)
	}
	...
	if _, err = writer.Write(data); err != nil {
		return nil, adError.Wrapf("failed to compress input data: err: {{err}}", err)
	}
...
func Decompress(data []byte) ([]byte, bool, error) {
	...
	if err != nil {
		return nil, false, adError.Wrapf("failed to create a compression reader: {{err}}", err)
	}
	if reader == nil {
